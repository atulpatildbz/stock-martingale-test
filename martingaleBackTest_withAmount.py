...
for idx, val in enumerate(closeArray):
    if(val >= exitsArray[buyNumber-1]):
        ...
        f.write('-----------------------------------\n')
        pl = getPL(lastBuy, val, buyQuantity)
    ...
    if(val <= buysArray[buyNumber]):
        ...
        f.write("\tAmount: "+str(initialInv*(pow(2,(buyNumber-1)))))
        f.write("\tpercent: "+str(round(((val-lastBuy)/lastBuy)*100,2))+"\n")
        buyQuantityCurrent = (initialInv**(buyNumber-1))//val
        buyAmountCurrent = buyQuantityCurrent*val
        lastBuy = val
        currentInv = currentInv + buyAmount
        bagOfMoney = bagOfMoney - buyAmount
        if(bagOfMoney < 0):
            print("GAME OVER")
            f.write("GAME OVER")
            break
